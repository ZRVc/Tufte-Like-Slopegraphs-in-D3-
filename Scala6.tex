...
\setmathfont[range=\mathup/{greek,Greek}]{Alegreya-Regular_0.otf}
\setmathfont[range=\mathit/{greek,Greek}]{Alegreya-Regular_0.otf}
...
\newfontfamily\myfontA[Numbers={Tabular,OldStyle}]{BEMBOBOOKMTSTD-REGULAR_0.otf}
...
\end{minipage}}
\adjustbox{valign=t}{\begin{minipage}[]{1\textwidth}
\vspace{-3.25cm} \includegraphics[]{TufteDrawFull1.eps}
\end{minipage}}%
\newpage
\adjustbox{valign=t}{\begin{minipage}[28.2cm]{0.53\textwidth}
$\vcenter{\hbox{\includegraphics[]{TufteCrash.eps}}}$
\end{minipage}}%
\hfill
...
The function to be optimized is the sum of squared distances between the points and their ideal placements.  The two columns of $n$ rows of data are combined into one vector, \textbf{x}.  Let \textbf{y} be the vector of the points' vertical positions.  Let \textbf{z} be the vector of their ideal positions.  The \textbf{objective function} is:

\begin{equation}
\sum_{i=1}^{2n}(z_i-y_i)^2
\end{equation}

The first constraint keeps the drop coefficient within some tolerance for each line.  Let \textbf{x}$\mathbf{_1}$ be the numbers in Column 1 and \textbf{y}$\mathbf{_1}$ the vector of their vertical positions.  Let \textbf{x}$\mathbf{_2}$ and \textbf{y}$\mathbf{_2}$ be the corresponding vectors for Column 2.  \textbf{Constraint 1:}

\begin{equation}
-\text{\myfontC є} \leq \frac{y_{1,i}-y_{2,i}}{x_{1,i}-x_{2,i}}\ - \frac{y_{1,i+1}-y_{2,i+1}}{x_{1,i+1}-x_{2,i+1}}\ \leq \text{\myfontC є}
\end{equation}

\textbf{Constraint 2} will make sure that the points stay in order, and that the minimum distance between them is $\zeta$ pixels, where $\zeta$ is chosen to provide the appropriate line spacing (I'm using $\zeta=16$ for an $11.5$ pt. font).  \textbf{y}$\mathbf{_1^*}$ and \textbf{y}$\mathbf{_2^*}$ will beginning\footnotemark.  With \textbf{x}$\mathbf{_1}$, \textbf{x}$\mathbf{_2}$, \textbf{y}$\mathbf{_1}$, and \textbf{y}$\mathbf{_2}$ as before:
...
\end{minipage}}
\footnotetext{\textsf{We need to use }\textbf{y}$\mathbf{_1^*}$\textsf{ and }\textbf{y}$\mathbf{_2^*}$\textsf{ for the condition rather than }\textbf{x}$\mathbf{_1}$\textsf{ and }\textbf{x}$\mathbf{_2}$\textsf{, because the starting points are all unique.}}
\newpage
...
\newpage
\lstinputlisting[language=R,deletekeywords={diff, drop,start,dir},otherkeywords={ROI_solve,L_constraint,OP,F_objective,solution,inequalitymaker,constraintmaker,ineq0,ineq1,ineq2,ineq3,fn,gr,fn1,gr1,adjuster,differ}]{TufteCode.R}
\end{document}
